...
const ko = {
  ...
  appName: "숨",
  /**
   * ⚠️ 예전 값은 "채널 → 소재 → 좌표" 였다 (2026-08-22 교체).
   *
   * 파이프라인 다이어그램이라 **우리가 어떻게 계산하는지**를 말하고 있었다.
   * 방문자가 알아야 하는 건 무엇을 얻는가다. `좌표` 도 우리 내부 말이다 —
   * 크리에이터는 촬영지를 찾지 좌표를 찾지 않는다.
   */
  appTagline: "소재로 찾는 촬영지",
  /**
   * ⚠️ "데모 데이터"만 적으면 처음 온 사람은 **서비스가 가짜**라고 읽는다.
