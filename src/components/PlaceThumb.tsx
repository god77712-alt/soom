...
export function PlaceThumb({
  ...
  for (let y = 0; y < ROWS; y++) {
    for (let x = 0; x < COLS; x++) {
      ...
      /**
       * ⚠️ 진하기를 올렸다 (2026-08-22). 예전 값(0.12~0.67)은 검정 배경에서
       *    **거의 안 보여서 칸이 비어 있는 것처럼 읽혔다.** 목록에서 사진 있는
       *    카드와 나란히 놓으면 사진 없는 쪽은 그냥 구멍이었다.
       *
       *    "빈 회색 박스를 두면 고장난 것처럼 보인다" 는 원래 판단은 맞다.
       *    다만 안 보이는 무늬는 빈 박스와 같다 — 있는 척만 한 셈이었다.
       */
      dots.push({
        x: (((x + 0.5) / COLS) * 100).toFixed(2),
        y: (((y + 0.5) / ROWS) * 100).toFixed(2),
        r: ((0.8 + s * 3.2) / 2).toFixed(2),
        o: (0.22 + s * 0.72).toFixed(2),
      });
    }
  }
