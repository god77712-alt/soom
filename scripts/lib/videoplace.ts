 ...
 *
 * ── 실측으로 정해진 것 ───────────────────────────────────
 *   상호명으로 찾기   20.2%  → 자기 지역 관문 통과 14.6%
 *   지명으로 찾기     33.4%   ← 2.3배. 지역이 기본 단위여야 한다
 *   지명 중 장소 특정 39.2%
 *   (2026-08-21 · 여행 검색 코퍼스 5,873편 기준)
 *
 * ⚠️ **모수는 `found_by` 로 갈라야 한다.** 채널 깊이 훑기로 받은 145,692편에는
 *    SBS 뉴스·TV조선·디글이 통째로 들어 있어 적중률이 2.8% 다. 섞어 평균을 내면
 *    3.2% 로 찍히는데, 매칭이 나쁜 게 아니라 **여행 영상이 아닌 걸 분모에 넣은 것**이다.
 *
...
/**
 * ⚠️ **채널 이름이 곧 장소 이름인 경우를 버린다.** (2026-08-21 추가)
 *
 * 자기 지역 관문을 통과하고도 남아 있던 마지막 오탐 무리다:
 *
 *   `KBC 광주방송` (서구)   88편   ← 채널 「KBC 콘텐츠」 의 영상마다 "광주" 가 나온다
 *   `제주살롱`   (제주시)   60편   ← 채널 「제주살롱」 의 영상마다 "제주" 가 나온다
 *
 * 둘 다 TourAPI 에 실제로 등록된 곳이라 이름은 정확히 맞고, 채널이 그 지역 채널이라
 * **지역 관문도 통과한다.** 그런데 영상은 그 장소를 찍은 게 아니라 그냥 자기
 * 채널명을 적은 것이다. 방송사 채널 하나가 장소 하나에 88편을 몰아준다.
 *
 * 규칙은 임계값이 아니라 사실 판정이다 — **매칭된 이름이 그 영상을 올린 채널의
 * 이름 안에 들어 있으면 버린다.** 지어낸 계수가 아니라서 안전하다.
 */
export function isChannelSelfMatch(channelTitle: string, matchedName: string): boolean {
  const c = norm(channelTitle ?? "");
  return c.length > 0 && c.includes(matchedName);
}
